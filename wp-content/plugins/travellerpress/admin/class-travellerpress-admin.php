...
class TravellerPress_Admin {
	...
	public function enqueue_admin_scripts() {
		...
		wp_register_script(
			'google-maps-js-api',
			'//maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=geometry,places"',
			array(),
			null
		);
	...
	public function add_map_meta_boxes( $post_type ) {
        ...
        if ( in_array( $post_type, $post_types )) {
        	add_meta_box(
        		'map-meta-el-boxes',
        		__( 'Map elements editor', 'foodiepress' ),
	...
	function travellerpress_settings_init(  ) { 
		...
		add_settings_field( 
			'travellerpress_text_field_0', 
			__( 'Saved map settings ', 'travellerpress' ), 
			array( $this, 'travellerpress_mapGeneral' ),
			'mapGeneral', 
			'travellerpress_mapGeneral_section' 
		);
	...
	function travellerpress_mapstyles(  ) { 
		...
		<p class="description">You can use custom map styles with Google Maps so you won't need to use always the same boring map. There are two big sites with pre-made styles: <a href="https://snazzymaps.com/">Snazzy Maps</a> and <a href="http://www.mapstylr.com/">Mapstylr</a>. Just copy one of the codes from that website and add here as your style</p><br><br>
	...
	function travellerpress_option_global_map_elements_callback(){
		echo "<p>This is a global map - each point on map is representation of single post (unless more than one post is set to the exact same location). Using this panel you can add additional polylines, polygones or KML files to this map.</p>";
	}

	function travellerpress_mapGeneral(  ) { 
		...
		<p class="description">If you feel your map is too cramped you can change the value of icons scale, default is 1.1, changing it to lower will make map icons smaller</p><br><br>
...
function tp_modify_maps_table( $column ) {
    $column['tp_maps_shortcode'] = 'Shortcode (copy&paste to post/page content)';
    return $column;
}
